...
def cambiar_jornada_api(request):
    ...
    try:
        ...
        # Fallback de posicion desde EstadisticasPartidoJugador (cuando ejt.posicion es NULL)
        pos_fallback = {}
        for row in (
            EstadisticasPartidoJugador.objects
            .filter(partido__jornada__temporada=temporada_actual, posicion__isnull=False)
            .exclude(posicion='')
            ...
            jid = row['jugador_id']
            if jid not in pos_fallback:
                pos_fallback[jid] = row['posicion']
        ...
        for ejt in EquipoJugadorTemporada.objects.filter(
            ...
            posicion = ejt.posicion or pos_fallback.get(jugador.id) or 'Delantero'
...
def explicar_prediccion_portero_api(request):
    ...
    try:
        ...
        # Pasar jornada como int si es posible
        ...
        if prediccion is not None and jornada is not None:
            try:
                # Mapear código de modelo al almacenado en BD
                _MODEL_MAP = {
                ...
                # Si modelo_tipo es None, usar el modelo por defecto de cada posición
        ...
        logger.error(f"[XAI API] JSON inválido: {e}")
        return JsonResponse({'status': 'error', 'error': f'JSON inválido: {e}'}, status=400)
    except ImportError as e:
        ...
        return JsonResponse({'status': 'error', 'error': f'Error de valor: {e}'}, status=400)
    except Exception as e:
