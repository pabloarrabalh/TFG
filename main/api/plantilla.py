...
class MiPlantillaJugadoresView(APIView):
    ...
    def get(self, request):
        ...
        if q:
            qs = qs.filter(Q(jugador__nombre__icontains=q) | Q(jugador__apellido__icontains=q))

        result = []
        for ejt in qs.order_by('jugador__apellido')[:50]:
            stats = EstadisticasPartidoJugador.objects.filter(jugador=ejt.jugador, partido__jornada__temporada=temporada).aggregate(pts=Sum('puntos_fantasy'))
            result.append({
                'id': ejt.jugador.id,
                'nombre': ejt.jugador.nombre,
                'apellido': ejt.jugador.apellido,
...
class PlantillasUsuarioView(APIView):
    """
    GET  /api/plantillas/usuario/ 
    POST /api/plantillas/usuario/ 
    """
    permission_classes = [IsAuthenticated]
...
class PlantillaItemView(APIView):
    """
    POST   /api/plantillas/usuario/<id>/renombrar/ 
    DELETE /api/plantillas/usuario/<id>/           
    """
    permission_classes = [IsAuthenticated]
