...
from .common import get_temporada_by_display, parse_int
from ..models import *
...
class EstadisticasView(APIView):
    ...
    def get(self, request):
        ...
        limit = parse_int(request.query_params.get('limit', 50), default=50, min_value=1)
        offset = parse_int(request.query_params.get('offset', 0), default=0, min_value=0)

        temporada = get_temporada_by_display(temporada_str.replace('_', '/'))
        if not temporada:
            return Response({'estadisticas': [], 'total': 0})

        stats_qs = EstadisticasPartidoJugador.objects.filter( partido__jornada__temporada=temporada).select_related('jugador', 'partido__equipo_local', 'partido__equipo_visitante')

        # Filtrar por jornada específica o rango de jornadas
        if jornada_num:
            jornada_num_int = parse_int(jornada_num, default=None, min_value=1)
            try:
                jornada = Jornada.objects.get(numero_jornada=jornada_num_int, temporada=temporada)
                stats_qs = stats_qs.filter(partido__jornada=jornada)
            except (TypeError, ValueError, Jornada.DoesNotExist):
                pass
        else:
            if jornada_desde:
                jornada_desde_num = parse_int(jornada_desde, default=None, min_value=1)
                if jornada_desde_num is not None:
                    stats_qs = stats_qs.filter(partido__jornada__numero_jornada__gte=jornada_desde_num)
            
            if jornada_hasta:
                jornada_hasta_num = parse_int(jornada_hasta, default=None, min_value=1)
                if jornada_hasta_num is not None:
                    stats_qs = stats_qs.filter(partido__jornada__numero_jornada__lte=jornada_hasta_num)

        if search:
            search_lower = search.lower()
            stats_qs = stats_qs.filter(Q(jugador__nombre__icontains=search_lower) | Q(jugador__apellido__icontains=search_lower))
        ...
        ejt_bulk = EquipoJugadorTemporada.objects.filter( temporada=temporada ).select_related('equipo').values(
