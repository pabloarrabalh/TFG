...
export default function NotificacionesBell() {
  ...
  async function fetchNotifs() {
    try {
      ...
      setUnread(res.data.no_leidas || 0)
    } catch {
      // silent
    }
  }
  ...
  return (
    <div ref={panelRef} className="relative">
      ...
      {/* Dropdown panel */}
      {open && (
        <div className="absolute right-0 top-12 w-[360px] z-50 bg-[#1a1d23] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
          {/* Header */}
            ...
            <div className="flex items-center gap-2">
              ...
              {unread > 0 && (
                <span className="bg-red-500/20 text-red-400 text-xs font-black px-2 py-0.5 rounded-full">{unread} nuevas</span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {unread > 0 && (
            ...
            {notifs.length === 0
              ? (
                <div className="py-10 text-center">
                  <span className="material-symbols-outlined text-4xl text-gray-600 block mb-2">notifications_off</span>
              ...
              : notifs.map(notif => {
                  ...
                  return (
                    <div key={notif.id}
                      className={`flex gap-3 px-4 py-3 border-b border-white/5 transition-colors ${notif.leida ? 'opacity-60' : 'bg-white/[0.03]'}`}>
                      {/* Icon */}
                      <div className={`flex-shrink-0 size-9 ${cfg.bg} rounded-xl flex items-center justify-center mt-0.5`}>
                        <span className={`material-symbols-outlined text-base ${cfg.color}`}>{cfg.icon}</span>
                      </div>

                      {/* Content */}
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-xs font-semibold leading-tight">{notif.titulo}</p>
                        <p className="text-gray-400 text-xs mt-0.5 leading-snug">{notif.mensaje}</p>
                        <p className="text-gray-600 text-[10px] mt-1">{timeAgo(notif.creada_en)}</p>

                        {/* Friend request actions */}
                        {isFriendReq && pendiente && !notif.leida && (
                          <div className="flex gap-2 mt-2">
                            <button onClick={() => acceptFriendRequest(notif)} disabled={loading}
                              className="flex items-center gap-1 px-2.5 py-1 bg-green-500/20 hover:bg-green-500/40 text-green-400 rounded-lg text-xs font-bold transition-colors disabled:opacity-60">
                              <span className="material-symbols-outlined text-xs">check</span>
                              Aceptar
                            </button>
                            <button onClick={() => rejectFriendRequest(notif)} disabled={loading}
                              className="flex items-center gap-1 px-2.5 py-1 bg-red-500/20 hover:bg-red-500/40 text-red-400 rounded-lg text-xs font-bold transition-colors disabled:opacity-60">
                        ...
                        {isFriendReq && !pendiente && (
                          <span className="mt-1 inline-block text-[10px] text-gray-500">
